...
def main():
    ...
    # prepare data
    with open(os.path.join(config.DATASET.ROOT, 'trainval_splits.json'), 'r') as f:
        dataset_splits = json.load(f)

    train_list = config.DATASET.TRAIN_TILES
    train_ds = []
    val_ds = []
    for i in range(len(train_list)):
        t_ds = eval('datasets.' + config.DATASET.READER)(
            config,
        ...
        train_ds.append(t_ds)

        v_ds = eval('datasets.' + config.DATASET.READER)(
            config,
            dataset_splits=dataset_splits,
            **train_list[i],
            fold_num=n_fold,
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if dist_helper.get_rank() == 0:
            ...
            scores_msg = ", ".join([f"{k}={v:.4f}" for (k, v) in scores.items() if np.isscalar(v)])
